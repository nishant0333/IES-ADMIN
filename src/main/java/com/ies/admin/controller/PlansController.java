...
public class PlansController {
	...
	public ResponseEntity<List<Plans>> activeStatus(@PathVariable String name,@PathVariable Boolean status){
		...
		return new ResponseEntity<List<Plans>>(plansService.activeStatus(name, status),HttpStatus.OK);
	}
	
	@PutMapping("/edit/{name}")
	public ResponseEntity<String> editPlan(@PathVariable String name,@RequestBody Plans plan){
		
		return new ResponseEntity<String>(plansService.editPlan(name, plan),HttpStatus.CREATED);
	}
	
	@GetMapping("/get")
	public ResponseEntity<List<Plans>> getAllPlans(){
		
		return new ResponseEntity<List<Plans>>(plansService.getAllPlans(),HttpStatus.OK);
	}
	
	@PostMapping("/create")
	public ResponseEntity<String> createPlan(@RequestBody Plans plan) {
		
		return new ResponseEntity<String>(plansService.createPlans(plan),HttpStatus.CREATED);
	}
